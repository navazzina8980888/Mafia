...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You are not authorised to use this bot:- @Mafia_033_082_120 ❄."
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "You Are On Cooldown ❌. Please Wait 1sec Before Running The /Mafia Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 1200:
                ...
                record_command_logs(user_id, 'Mafia', target, port, time)
            ...
            response = "✅ Usage :- /Mafia <target> <port> <time>"  # Updated command syntax
    else:
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /Mafia command.                                                                          DM FOR GET ACCESS:- @Mafia_033_082_120")
...
💥 /Mafia : Method For MAFIA Servers. 
...
Get Access From:- @Mafia_033_082_120
'''
